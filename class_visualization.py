...
def create_class_visualization(target_y, model, dtype, init_img=None, l2_reg=1e-3,
                               ...
                               max_jitter=16, show_every=25, class_names=None,
                               savepath=None):
    """
    ...
    Args:
        ...
        model (torch.nn.Module): a pretrained CNN that will be used to generate the image
        ...
        blur_every (int): how often to blur the image as an implicit regularizer (if None, does not blur the image)
        max_jitter (int): how much to jitter the image as an implicit regularizer (if None, does not jitter the image)
